...
from modules.agents.agents_models import AgentPublic, AgentCreate, AgentUpdate
...
class AgentsController:
    ...
    def create_request(self, requset: Request, db: Session, data: AgentCreate):
        ...
        self.agents_service.create(db=db, agent=agent_data)
    ...
    def resource_request(self, request: Request, db: Session, agent_id: uuid.UUID):
        ...
        agent_resource: AgentPublic =  self.http_service.request_validation_service.verify_resource(
            "agents_service",
            {"db": db, "agent_id": agent_id},
            "Agent not found"
        )

        self.http_service.request_validation_service.validate_action_authorization(user.user_id, agent_resource.userId)
    ...
    def collection_request(self, request: Request, db: Session):
        ...
        data = self.agents_service.collection(db=db, user_id=user.user_id)
    ...
    def delete_request(self, request: Request, db: Session, agent_id: uuid.UUID):
        ...
        self.agents_service.delete(db=db, agent_id=agent_resource.agentId)
